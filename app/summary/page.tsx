...
export default function ActivityDashboard() {
  ...
  // Imposta il range di default (ultimi 3 mesi) solo lato client per evitare hydration mismatch
  useEffect(() => {
  ...
  const [users, setUsers] = useState<
    Array<{ id: string; email: string; role: string; auth_id: string | null }>
  >([]);
  const [activeTab, setActiveTab] = useState<"operatori" | "attivita">(
    "operatori",
  );
  const [selectedRole, setSelectedRole] = useState<
    "operator" | "admin" | "referrer" | "all"
  >("operator");
  ...
  useEffect(() => {
    ...
    async function loadActivities() {
      try {
        ...
        if (userError)
          throw new Error("Errore di autenticazione: " + userError.message);
        ...
        if (profileError)
          throw new Error(
            "Errore nel recupero del profilo: " + profileError.message,
          );
        if (!profile || !["referrer", "admin"].includes(profile.role)) {
          throw new Error(
            "Accesso non autorizzato: solo i referenti e gli amministratori possono visualizzare questa pagina",
          );
        }
        ...
        if (activitiesError)
          throw new Error(
            "Errore nel recupero delle attività: " + activitiesError.message,
          );
  ...
  const filteredActivities = activities.filter((activity) => {
    const matchesUser =
      ...
      (activity.profile?.email &&
        selectedUsers.includes(activity.profile.email));
  ...
  return (
    <div className="flex-1 space-y-6 p-6">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as "operatori" | "attivita")}
      >
